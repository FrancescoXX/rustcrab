...
const DSAPage: React.FC = () => {
	...
	const handleSearch = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		setSearchTerm(term);
		const filtered = dsas.filter((dsa) => dsa.data_structure.toLowerCase().includes(term) || dsa.description.toLowerCase().includes(term));
	...
	return (
		<div className='min-h-screen bg-gray-100 dark:bg-gray-900'>
			<div className='container mx-auto px-4 py-12'>
				<h1 className='text-4xl font-bold mb-8 text-center text-gray-800 dark:text-white'>Data Structures in Rust</h1>
				...
				<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
					{filteredDSAs.map((dsa, index) => (
						<div key={index} className='bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg transition duration-300 hover:shadow-xl hover:scale-105'>
							<div className='flex flex-col items-center text-center'>
								<h3 className='text-2xl font-semibold mb-3 text-gray-800 dark:text-white'>{dsa.data_structure}</h3>
								<p className='text-gray-600 dark:text-gray-400 mb-4'>{dsa.description}</p>
								<button
				...
				{filteredDSAs.length === 0 && (
					<p className='text-center text-gray-600 dark:text-gray-400 mt-8'>No data structures found matching your search.</p>
				)}
			</div>
				...
				<div className='fixed inset-0 flex items-center justify-center z-50 p-4 sm:p-8'>
					...
					<div className='bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg z-50 max-w-full sm:max-w-3xl w-full'>
						<div className='flex justify-between items-center mb-4 ml-3 mr-1'>
							<h3 className='text-2xl font-semibold dark:text-white  text-current'>{selectedTool.data_structure}</h3>
						...
						<pre className='bg-gray-100 dark:bg-slate-900  rounded-md overflow-auto max-h-96'>
							<code className='text-left '>
								<SyntaxHighlighter language='Rust' style={nightOwl} className='rounded-lg'>
